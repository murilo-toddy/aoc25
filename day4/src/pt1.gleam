...
fn count_neighbors(in: Grid, row: Int, col: Int) -> Int {
    ...
    let neighbors = list.flatten(list.map(directions, fn(dir1) {
    ...
    let res = list.fold(neighbors_count, 0, fn(i, acc) { acc + i })
    res
}

fn count_rolls_rec(grid: Grid, row: Int, col: Int, acc: Int, rows: Int, cols: Int) -> Int {
    ...
    case #(row, col) {
        #(row, col) if row < rows && col < cols -> {
            let count = accessible(grid, row, col)
            count_rolls_rec(grid, row, col + 1, acc + count, rows, cols)
        }
        #(row, col) if row < rows && col >= cols -> {
            let count = accessible(grid, row, col)
            count_rolls_rec(grid, row + 1, 0, acc + count, rows, cols)
        }
        _ -> acc
    }
}

fn count_rolls(grid: Grid, rows: Int, cols: Int) -> Int {
